...
import { ModalInfoTerapComponent } from 'src/app/cuestionario/modal-info-terap/modal-info-terap.component';
...
import { TerapItem } from 'src/app/models/terap-item';
...
export class EditarCuestionarioComponent implements OnInit {
    ...
    if (this.tokenService.getToken()) {
      ...
      this.cuestionarioService.details(this.claveCuestionario).subscribe(
        data => {
          ...
          this.apellido = data.apellido; //apellido cuestionario
          this.peso = data.peso; //peso cuestionario
          this.talla = data.talla; //talla cuestionario
          this.imc = data.imc; //imc cestionario
          this.f_nac = data.fechaNacimiento; //fecha nacimiento cuestionario
          this.l_nac = data.ciudadNacimiento; //lugar nacimiento cuestionario
          this.obsClin = data.obsClin; // observaciones clinicas 
          this.datosLabo = data.datosLabo; // datos laboratorio
          this.opSangSel = data.tipoSangre;
          this.gradoInfo = data.gradoInfo;
          this.opAlcoholSel = data.alcoholismo;
          this.opTabSel= data.tabaquismo;
          this.opDrogSel= data.drogas;
          this.opSuplSel= data.suplementos; 
          this.opHerbSel= data.herbolaria;
          this.opMedTradSel= data.medicinaTradicional;

          let padeString = [];
          data.padecimientos.forEach(padecimiento => {
            this.lang() == "es" ? padeString.push(padecimiento.nombreEs) : this.lang() == "en" ? padeString.push(padecimiento.nombreEn) : padeString.push(padecimiento.nombreBr);
          });
          this.padecimientosList = padeString;
          let medraString = [];
          data.medraList.forEach(med => {
            this.lang() == "es" ? medraString.push(med.medDraEs) : this.lang() == "en" ? medraString.push(med.medDraEn) : medraString.push(med.medDraBr);
          });
          this.medraList = medraString;

          let terapItems = [];
          data.terapeuticas.forEach(terap => {
            terapItems.push(new TerapItem(
              terap.medicamento.productName,
              terap.distintivo,
              terap.presentacion,
              terap.dosis,
              terap.via,
  ...
  public cargarPadecimientos(letra: string) {
  ...
  public cargarMedicamentos(letra: string) {
    this.options = [];
